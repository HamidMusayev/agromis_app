...
class _SelectGardenScreenState extends State<SelectGardenScreen> {
  ...
  getGardens() async {
    await ListOperations.getGardenList().then((value) => value != false ? setState(() => gardens = value) : null)
        .then((value) => setState(() => loading = false));
  }

  getPrefs() async {
    await SharedData.getBool("tipsOpen")
        .then((value) => setState(() => tips = value));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(Constants.tSelectGarden, style: semibold16Style),
      ),
      body: Column(
        children: <Widget>[
          tips != null && tips
              ? InfoCard(text: Constants.tInfo3Text)
              : Container(),
          Padding(
          ...
          loading
              ? CustomLoading()
              : Expanded(
                  child: _searchResult.length != 0 || searchTxt.text.isNotEmpty
                      ? ListView.builder(
                          ...
                          itemBuilder: (context, i) {
                            return ListTile(
                              ...
                              onTap: () {
                                Navigator.push(context, MaterialPageRoute(builder: (context) => SelectTreeScreen()));
                              },
                            );
                          },
                        )
                                            ...
                                            title: Column(
                                              children: <Widget>[
                                                      ...
                                                      children: <Widget>[
                                                        TextFormField(
                                                          ...
                                                          decoration: InputDecoration(
                                                            ...
                                                            prefixIcon: Icon(Icons.border_top_rounded),
                                                            enabledBorder: UnderlineInputBorder(borderSide: BorderSide(color: kInputTextColor, width: 1.5)),
                                                            focusedBorder: UnderlineInputBorder(borderSide: BorderSide(color: kPrimaryColor, width: 1.5)),
                                                        ...
                                                        TextFormField(
                                                          ...
                                                          decoration: InputDecoration(
                                                            ...
                                                            prefixIcon: Icon(Icons.border_left_rounded),
                                                            enabledBorder: UnderlineInputBorder(borderSide: BorderSide(color: kInputTextColor, width: 1.5)),
                                                            focusedBorder: UnderlineInputBorder(borderSide: BorderSide(color: kPrimaryColor, width: 1.5)),
                                                            fillColor: kWhiteColor,
                                                          ),
                                                ...
                                                DefaultButton(
                                                  ...
                                                  onPress: () {
                                                    if (ysiraTxt.text.trim().isNotEmpty) {
